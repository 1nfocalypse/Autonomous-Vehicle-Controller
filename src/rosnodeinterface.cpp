...
// C/C++ source code generated on : Wed Nov 29 23:36:53 2023
//
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"okayestcontroller\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(okayestcontroller_M, (NULL));
        okayestcontroller_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(okayestcontroller_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(okayestcontroller_M)
                == (NULL)) || rtmGetStopRequested(okayestcontroller_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(okayestcontroller_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
