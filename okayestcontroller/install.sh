...
echo "=========================="
echo "Installing App okayestcontroller"

# Here is where we perform installation of scripts, services, etc.
echo " - Installing ROS packages for okayestcontroller..."
...
runuser -l $LIBPANDA_USER -c /etc/libpanda.d/apps/okayestcontroller/installRosPackages.sh
...
rosrun robot_upstart install okayestcontroller/launch/${LAUNCH_FILE} --user root

echo "Enabling can_to_ros startup script"
sudo systemctl daemon-reload
sudo systemctl enable okayestcontroller
popd
echo "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
